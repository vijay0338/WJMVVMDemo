//
//  WJCategoryRightTableViewCell.m
//  WJWineDemo
//
...
@interface WJCategoryRightTableViewCell()
...
@property(nonatomic,strong)WJGood              *goods;

///加号减号
@property(nonatomic,strong)WJGoodManagerView   *managerView;
@end


@implementation WJCategoryRightTableViewCell
...
- (void)bind
{
    ...
    [self.managerView.addSubject subscribeNext:^(id x) {
        @strongify(self);
        [WJTools beginAddAnimationWithImageView:self.w_imageView animationTime:0.55 startPoint:CGPointZero endPoint:CGPointZero];
    }];
}
...
{
    //  检查购物车是否有商品
    if ([[WJShoppingManager manager].goodsDic objectForKey:goods.id])
    {
        WJGood *good = [[WJShoppingManager manager].goodsDic objectForKey:goods.id];
        goods.num = good.num ;
    }
    else
    {
...
- (void)configView
{
    ...
    _w_titleLabel.textColor     = WJCOLOR(70, 70, 70, 1);
    ...
    _w_specLabel.textColor      = WJCOLOR(150, 150, 150, 1);
    ...
    self.managerView            = [[WJGoodManagerView alloc]init];
