...
- (void)initViewModel
{
    ...
    self.refreshCommand = [[RACCommand alloc]initWithSignalBlock:^RACSignal *(id input) {
        ...
        RACSignal *signal1 = [WJRequestManager getWithURL:@"http://www.jiuyunda.net:90/api/v1/product/slideshow" withParamater:@{@"id":CURRENT_USER.bid}];
        [signal1 subscribeNext:^(id x) {
            NSLog(@"%@",x);
        }];
        RACSignal *signal2 = [WJRequestManager postArrayDataWithURL:Home_Data withpramater:nil];
    ...
    self.btnCommand     = [[RACCommand alloc]initWithSignalBlock:^RACSignal *(id input) {
        ...
        switch (tag) {
            ...
            case 101:
            {
                WJRecommendViewModel *viewModel = [[WJRecommendViewModel alloc]initWithService:self.services params:@{@"title":@"推荐有奖"}];
                self.naviImpl.className = @"WJRecommendVC";
...
//        WJGoodsViewModel *viewModel = [[WJGoodsViewModel alloc]initWithService:nil params:@{@"title":@"商品详情"}];
...
#warning 以后修改
    ...
    if ([headArray[@"code"] integerValue] == 100)
    {
        ...
        {
            WJGood *good = [[WJGood alloc]initWithDic:dic];
