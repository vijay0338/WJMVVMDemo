...
@implementation WJGoodManagerView
...
- (void)leftBtnClick:(UIButton *)btn
{
    ...
    if (self.goods.num == 0)
    {
        [[WJShoppingManager manager].goodsDic removeObjectForKey:self.goods.id];
        [self.reduceSubject sendNext:self.goods.id];
    }
    else
    {
        [[WJShoppingManager manager].goodsDic setObject:self.goods forKey:self.goods.id];
    }
    [WJUser currentUser].bageValue--;
}
