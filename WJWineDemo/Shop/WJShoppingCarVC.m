...
#import "WJEmptyShoppingCarView.h"


@interface WJShoppingCarVC ()
@property(nonatomic,strong)WJShoppingCarViewModel  *viewModel;

@property(nonatomic,strong)WJShoppingCarTableView  *tableView;
...
///购物车为空时
@property(nonatomic,strong)WJEmptyShoppingCarView  *emptyView;
@end

@implementation WJShoppingCarVC
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName:WJCOLOR(70, 70, 70, 1)};
...
- (void)initView
{
    ...
    _bottomView.backgroundColor = WJCOLOR(70, 70, 70, 0.8);
    [self.bottomView addSubview:self.priceLabel];
    [self.bottomView addSubview:self.payBtn];
    [self.bottomView addSubview:self.selectAllBtn];
    self.priceLabel.textColor   = WJCOLOR(252, 252, 252, 1);
...
- (UITableView *)tableView
{
    if(!_tableView)
    {
        _tableView = [[WJShoppingCarTableView alloc]initWithFrame:CGRectMake(0, 0, kWidth, kHeight - 40) style:UITableViewStylePlain];
...
- (WJEmptyShoppingCarView *)emptyView
{
    if (!_emptyView)
    {
        _emptyView = [[WJEmptyShoppingCarView alloc]initWithFrame:CGRectMake(0, 0, kWidth, kHeight)];
    }
    return _emptyView;
}
