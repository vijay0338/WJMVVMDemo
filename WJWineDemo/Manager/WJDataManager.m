...
+ (void)saveUserData
{
    if ([NSKeyedArchiver archiveRootObject:[WJShoppingManager manager] toFile:[NSHomeDirectory() stringByAppendingPathComponent:kShoppingPath]])
    {
        NSLog(@"购物车数据保存成功");
    }
    
    if([NSKeyedArchiver archiveRootObject:[WJUser currentUser] toFile:[NSHomeDirectory() stringByAppendingPathComponent:kUserPath]])
    {
        NSLog(@"用户数据保存成功");
    }
...
+ (void)readUserData
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (!shopError)
        {
            WJShoppingManager *manager = [NSKeyedUnarchiver unarchiveObjectWithData:shopData];
            [self setObj:[WJShoppingManager manager] fromObj:manager];
            NSLog(@"读取购物车数据成功");
        }
        ...
        if (!userError)
        {
            WJUser *user = [NSKeyedUnarchiver unarchiveObjectWithData:userData];
            [self setObj:[WJUser currentUser] fromObj:user];
